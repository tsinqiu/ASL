...
ONNX export, and Raspberry Pi deployment notes.
...
## Small v2 max_len=128 + Augmentation

This experiment uses `max_len=128` cached features and lightweight cached feature
augmentation to reduce the train/valid gap. The previous Small v2 `max_len=128`
reference result was best `valid_acc` around `0.5368`.

The augmentation is applied only during training. Validation and evaluation use
the cached features unchanged.
...
After export, copy `raspi_deploy\model.onnx` into the deployment branch. Do not
commit `.pt` checkpoints to the deployment branch.

## Raspberry Pi Deployment

The clean Raspberry Pi package lives in the deployment branch under
`raspi_deploy/`. It uses:

- ONNX Runtime inference
- MediaPipe landmark extraction
- CSI camera support through Picamera2
- recording-based isolated sign recognition
- CLI top1/top5 output with optional Chinese label meanings

Runtime example on Raspberry Pi:
...
- Lightweight cached feature augmentation
- ONNX export and check scripts
